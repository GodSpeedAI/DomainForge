...
`domainforge-core` exposes feature flags that enable bindings and CLI components:

- `cli`: builds the `sea` binary.
- `python`: enables PyO3 bindings (`domainforge`).
- `typescript`: enables napi-rs bindings (`@godspeedai/domainforge`).
- `wasm`: builds WASM targets.
- `serde`: serialization helpers (enabled by default).

Use `cargo build -p domainforge-core --features "cli python"` to combine features; omit unneeded ones for faster builds.

## Python package (`domainforge`)
...
## TypeScript package (`@godspeedai/domainforge`)
...
- **TypeScript**: `node_modules/@godspeedai/domainforge` native addon; resolves platform-specific `.node` files.
...
- name: Validate DSL fixtures
  run: |
    domainforge validate docs/examples/payment.sea --format json
```
...
`domainforge-core` uses `env_logger` when built with the CLI feature and forwards logs through bindings.

- Levels: `trace`, `debug`, `info`, `warn`, `error`.
- Enable timestamps: set `RUST_LOG_STYLE=always` when using the CLI; bindings use the default style.
- Example: `SEA_LOG=debug domainforge validate model.sea` prints parser tokens and validator decisions.
...
- Avoid piping untrusted CALM/DSL directly into production graphs without validation; always run `domainforge validate` first.
...
| --- | --- | --- |
| CI validation | `domainforge validate --format json` | Combine with `jq` to fail on violations. |
| CALM export | `domainforge project --format calm --out calm.json` | Inspect metadata for `sea:version`. |
| Policy eval with unknowns | `sea eval --allow-unknown` | Mirrors three-valued logic. |
| Local doc testing | `SEA_LOG=debug domainforge parse docs/examples/*.sea` | Useful when writing tutorials. |
...
- `domainforge-core/src/cli/mod.rs` — defines CLI arguments and defaults.
- `domainforge-core/src/python/` — Python binding constructors that accept flags.
- `domainforge-core/src/typescript/` — TypeScript binding options for parse/evaluate.
