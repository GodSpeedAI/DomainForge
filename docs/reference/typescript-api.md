...
```typescript
import { formatSource, checkFormat } from "domainforge";
...
```typescript
import { Expression, BinaryOp } from "domainforge";
...
The `domainforge` module exposes semantic pack enums and functions for building, validating, signing, and inspecting packs programmatically.
