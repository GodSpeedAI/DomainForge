...
|---|---|---|
| Graph | Existing entities, roles, policies, patterns, resources, and flows are constructed from the shared resolved symbol payload and retain `ConceptId`. Graph carries resolved `EntityContract` and referenced `EnumContract` indexes solely to validate concrete entity instances. It does not own records or operations. | Milestone 0 must prove Graph and `ApplicationContract` resolve the same existing-concept IDs; graph construction validates typed instances without enforcing application-only operation contracts. |
