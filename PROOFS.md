...
| CLAIM-PROJ-CELL | Cell projection emits a hermetic environment (Devbox + Mise + dependency-set + sandbox/network + authority + evidence + `cell.lock`) that is byte-deterministic and validates natively | partial | `src/projection/cell/` | `fixtures/cell_env/repair-agent/model.sea` | `bash scripts/verify/cell.sh` | Two-run `diff -r` + structural JSON parse of every artifact; `devbox generate direnv` and `mise config` run natively when those CLIs are on `PATH` (confirmed passing locally, including a real nixpkgs `cacert` resolution) | none yet — not wired into `evidence/latest/fragments/projections.json` | Two runs differ, a JSON artifact fails to parse, or a present native CLI rejects its manifest | `verify-cell` CI job does not install `devbox`/`mise`, so CI itself only proves byte-determinism + structural validity, not the native toolchains; not yet folded into `just prove`/`scripts/prove/projections.sh` |
