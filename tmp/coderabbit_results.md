...
[ ] Task:
In docs/plans/Enhancements-Hardening-plan.md around line 16 (and likewise lines 41, 69, 93), the Status markers currently show "✅ Implemented" which conflicts with the document's planning tone; update the status labels to clearly indicate intent: if this is a prospective plan, change statuses to planned/in-progress symbols (e.g., "⬜ Planned", "🔄 In Progress") and remove file/line implementation references; if it documents completed work, keep "✅ Implemented" but add a date/version and brief note referencing the commit or release that completed it; make the same change consistently at the other referenced lines (41, 69, 93).
...
[ ] Task:
In docs/plans/Enhancements-Hardening.md around line 164, the file currently self-references "Full plan: docs/plans/Enhancements-Hardening.md (generated by plan_task tool)"; remove or replace this self-referential link with either a substantive external reference (e.g., a different doc or tooling README) or rephrase to clarify intent (for example, "This plan was generated by the plan_task tool — see docs/tools/plan_task.md for details"); update the line accordingly and ensure any referenced path actually points to an existing file.
...
[ ] Task:
In docs/plans/tmp/task-06-rdf-sbvr-escaping.md around lines 655 to 658, the TypedLiteralSuffix enum is currently declared in the wrong scope; make it accessible where parse_typed_literal() is defined/used (lines ~605–606 and 662–711) by either moving the enum into the impl block if parse_typed_literal() is an impl method, or by defining it at module level with appropriate visibility (add pub if used from other modules); update any references/imports accordingly so the enum is in scope for parse_typed_literal().
...
[ ] Task:
.pre-commit-config.yaml around line 44: the exclude regex currently is unanchored and can match substrings (e.g. my_target.ts); update the pattern to anchor directory boundaries so it only excludes those top-level directories or path segments — replace the current pattern with one that requires a start or slash before the directory name and a slash or end after it (e.g. use a pattern like '(^|/)(node_modules|pkg|target|sea-core/target)(/|$)').
...
[x] Done:
Updated the test examples to call `Entity::new_with_namespace(..., "default")` (and analogous `Resource::new_with_namespace` where relevant). Verified `cargo test -p sea-core` — tests pass.
...
[ ] Task:
In sea-core/src/primitives/resource.rs around lines 90 to 93, the doc example asserts resource.unit().symbol() == "kg" but the Unit::new call uses "kilogram" as the symbol argument, so the assertion is wrong; update the doc example to assert the actual symbol (change the assertion to assert_eq!(resource.unit().symbol(), "kilogram")) or alternatively swap the Unit::new arguments so the symbol is "kg" to match the original assertion—prefer updating the assertion to "kilogram" to match the current constructor call.
...
[ ] Task:
In sea-core/fix_api_v2.py around lines 168 to 170 in EntityTransformer._find_entity_new_calls, the code accesses function_node text without verifying its type like the other block at lines 253–256; add the same defensive check: ensure function_node is truthy and function_node.type == 'scoped_identifier' before calling self._node_text(function_node).decode() and comparing to 'Resource::new', so the method mirrors the robustness used at lines 253–256.
