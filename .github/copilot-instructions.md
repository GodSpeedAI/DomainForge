...
If you'd like, I can expand any section (debugging, CI, parser examples, or cross-language testing procedures) into a short, runnable checklist.
...
These instructions are for AI coding agents (Copilot/Coder) working on DomainForge.
Keep it concise: prefer `sea-core/*` as the canonical source, and keep changes in the Rust core consistent across the FFI layers.

## Quick Overview

- Canonical implementation: `sea-core/` (Rust). All language bindings (Python, TypeScript, WASM) wrap the Rust core.
- Five primitives: Entity, Resource, Flow, Instance, Policy (see `sea-core/src/primitives/`).
- Deterministic iteration via `IndexMap` in graph storage (`sea-core/src/graph/mod.rs`).

## Critical workflows & commands (copy-paste)

- Use `just` tasks defined in the repo root for common operations:

  - `just rust-test` — run Rust tests (uses `--features cli`)
  - `just python-test` — run Python tests (prefers .venv)
  - `just ts-test` — run TypeScript tests (Vitest)
  - `just all-tests` — run all three suites
  - `just prepare-rust-debug` — build and symlink Rust test binary for debugging
  - `just python-setup` — prepare virtualenv and install dev dependencies

- Rust builds frequently require feature flags for bindings:
  - Python: `cargo build --features python` or use `maturin develop --features python` (see `just python-setup`).
  - TypeScript: `cargo build --features typescript` or run `npm run build`.
  - WASM: `cargo build --features wasm` and `./scripts/build-wasm.sh`.

## Editing / Adding Core Logic

- Rust is canonical: When changing `sea-core` types or semantics, update all FFI layers (`sea-core/src/python`, `sea-core/src/typescript`, `sea-core/src/wasm`) and their tests.
- When changing grammar (DSL parser): update `sea-core/grammar/sea.pest`, then `parser` AST and tests (`tests/test_parser.py`).

## Conventions & Patterns Agents Must Know

- Determinism: `IndexMap` is used intentionally to avoid test flakiness and policy non-determinism.
- Ordering of operations: Add Entities/Resources before Flows/Instances; Graph enforces referential integrity.
- Use `Uuid::new_v4()` for IDs, and `rust_decimal::Decimal` for quantities (not f64).
- `namespace()` now returns `&str` with default "default" — check for empty string rather than `Option`.
- Unit conversion: use `sea_core::units::unit_from_string` and `Unit::new()` helpers.
- `Flow::new` and similar constructors accept prefer `ConceptId` (IDs) rather than references.

## Bindings & API Stability

- All bindings (PyO3, napi-rs, wasm-bindgen) delegate to `sea_core` — do not duplicate business logic.
- If you modify primitive fields, update and run these checks:
  1. Rust unit tests (cargo test)

2.  Python tests (pytest under `.venv`)
3.  TypeScript tests (npm test)
4.  CALM round-trip tests (cargo test calm_round_trip)

## Testing & CI

- The repo uses multi-language TDD: keep parity between Rust, Python, and TypeScript implementations.
- Use `just all-tests` locally to validate changes across languages.
- For Rust debugging: `just prepare-rust-debug` then attach `codelldb` to the symlinked binary.

## PR / Change Guidance

- If you touch `sea-core` primitives or the DSL grammar, update the Parser AST, FFI bindings, and add/modify tests for each language.
- Run `cargo clippy -- -D warnings` and `cargo fmt` before opening a PR.
- Add ADR updates under `docs/diagrams/ard/` for any architectural decisions that impact cross-language parity.

## Where to find more context

- Project architecture and plans: `docs/plans/*`, `docs/specs/*`.
- Solidity for grammar: `sea-core/grammar/sea.pest` and `sea-core/src/parser/`.
- Examples and usage: `examples/` and `python/sea_dsl` module.

## Common mistakes to avoid

- DO NOT duplicate business logic in FFI bindings — call into `sea_core`.
- ALWAYS update all bindings & tests whenever core data structures, function signatures, or grammar changes.
- DO NOT rely on non-deterministic iteration (HashMap) for policies; use the Graph API and IndexMap.

## Recommended agent behaviour

- Prefer small, testable changes and verify with `just` tasks.
- Use `just python-setup` in CI-like flows to ensure Python venv and `maturin` builds are available.
- When investigating complex changes, delegate deep file analysis or comparator work to subagents and return a concise decision/rationale.

---

Please review and tell me if you want additional examples or more detailed CI and debugging steps for any language (Rust/Python/TypeScript/WASM).
