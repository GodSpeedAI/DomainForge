use crate::application::envelope::resolve_semantic_envelope;
...
/// Resolve and print one file's Canonical Semantic Envelope document
/// (`domainforge-semantic-envelope/v1`) — the lossless canonical projection,
/// including every declared instance and policy. Replaces the hand-built
/// `application-contract-harness.rs` pattern: no other subcommand reaches
/// `resolve_semantic_envelope`.
#[derive(Parser)]
...
pub fn run(args: EnvelopeArgs) -> Result<()> {
    ...
    match resolve_semantic_envelope(&entry_logical_path, &sources_json) {
        Ok(doc) => {
            println!(
                "{}",
                serde_json::to_string_pretty(&doc)
                    .context("Failed to serialize semantic envelope")?
            );
            Ok(())
        }
        Err(diagnostics) => {
            ...
            Err(anyhow::anyhow!("Semantic envelope resolution failed"))
        }
    }
}
