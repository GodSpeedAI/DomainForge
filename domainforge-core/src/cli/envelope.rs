...
pub struct EnvelopeArgs {
    ...
    /// What to emit
    #[arg(long, value_enum, default_value_t = EmitMode::Representation)]
    pub emit: EmitMode,
    ...
    /// Path to namespace registry file (defaults to discovery from entry)
    #[arg(long)]
    ...
    /// Print adapter capabilities as JSON and exit
    #[arg(long)]
...
pub fn run(args: EnvelopeArgs) -> Result<()> {
    ...
    let entry = args
        .entry
        .as_ref()
        .expect("entry path required unless --capabilities is specified");

    let source = match fs::read_to_string(entry) {
        Ok(source) => source,
        Err(error) => {
            eprintln!("error: failed to read {}: {error}", entry.display());
            exit(2);
        }
    };
    ...
    let (entry_logical_path, sources) = match source_map_from_filesystem(
        ...
        Err(diagnostics) => {
            ...
            let logical = entry.display().to_string();
            emit_failed(
                &args,
                FailureContext {
