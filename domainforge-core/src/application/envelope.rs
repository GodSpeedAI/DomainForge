...
pub fn build_cep_envelope(p: &CepEnvelopeParams<'_>) -> serde_json::Value {
    ...
    match p.doc {
        ...
        None => {
            // F-03: A valid failure envelope is conformant to CEP-0008,
            // with representation_unavailable omission and model status invalid.
            envelope.insert(
                "conformance_status".to_string(),
                serde_json::json!("conformant"),
            );
            envelope.insert(
                "validation_status".to_string(),
                serde_json::json!("validated"),
...
/// Typed identity struct for a domain model across interface boundaries (F-04).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DomainModelIdentity {
    ...
    pub content_hash: String,
    pub semantic_closure_hash: String,
    pub semantic_pack_set_hash: Option<String>,
    pub registry_content_hash: Option<String>,
}

impl DomainModelIdentity {
    pub fn canonical_digest(&self) -> String {
        let value = serde_json::to_value(self).expect("DomainModelIdentity serializes");
        compute_sha256(canonical_json(&value).as_bytes())
    }
}
