...
use domainforge_core::{ErrorCode, ValidationError};
...
// Formatter tests
use domainforge_core::error::{DiagnosticFormatter, HumanFormatter, JsonFormatter, LspFormatter};
...
fn test_lsp_formatter_output() {
    ...
    assert_eq!(parsed["code"], "E005");
    assert_eq!(parsed["source"], "domainforge");
...
fn test_format_multiple_errors_json() {
    use domainforge_core::error::diagnostics::format_errors_json;

    let errors = vec![
        ValidationError::syntax_error("error 1", 1, 1),
        ValidationError::undefined_entity("Test", "line 5"),
        ValidationError::unit_mismatch(
            domainforge_core::units::Dimension::Mass,
            domainforge_core::units::Dimension::Currency,
            "line 10",
        ),
    ];
