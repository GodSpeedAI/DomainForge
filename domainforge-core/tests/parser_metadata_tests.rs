...
use domainforge_core::parser::ast::FileMetadata;
use domainforge_core::parser::{parse_source, parse_to_graph};
...
fn test_policy_metadata_applied_to_graph() {
    ...
    assert_eq!(
        policy.kind,
        domainforge_core::policy::PolicyKind::Constraint
    );
    assert_eq!(
        policy.modality,
        domainforge_core::policy::PolicyModality::Obligation
    );
