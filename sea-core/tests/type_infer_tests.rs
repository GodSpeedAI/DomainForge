...
fn test_infer_literal_types() {
    let graph = Graph::new();
    assert_eq!(infer_expression_type(&Expression::literal(serde_json::json!(true)), &graph), ExpressionType::Boolean);
    assert_eq!(infer_expression_type(&Expression::literal(serde_json::json!(42)), &graph), ExpressionType::Numeric);
    assert_eq!(infer_expression_type(&Expression::literal(serde_json::json!("x")), &graph), ExpressionType::String);
}
...
fn test_infer_binary_types() {
    ...
    assert_eq!(infer_expression_type(&expr, &graph), ExpressionType::Boolean);
...
fn test_infer_quantifier_and_aggregation() {
    let graph = Graph::new();
    let q = Expression::quantifier(sea_core::policy::Quantifier::ForAll, "e", Expression::variable("entities"), Expression::literal(serde_json::json!(true)));
    assert_eq!(infer_expression_type(&q, &graph), ExpressionType::Boolean);

    let agg = Expression::aggregation(AggregateFunction::Count, Expression::variable("entities"), Some("id".to_string()), None);
