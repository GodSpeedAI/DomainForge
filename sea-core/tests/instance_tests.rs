...
fn test_instance_references_are_valid_uuids() {
    ...
    assert!(instance.resource_id().to_string().len() > 0);
    assert!(instance.entity_id().to_string().len() > 0);
}
...
fn test_instance_attributes() {
    ...
    assert_eq!(instance.get_attribute("serial_number"), Some(&serde_json::json!("SN12345")));
    assert_eq!(instance.get_attribute("manufacture_date"), Some(&serde_json::json!("2025-01-15")));
